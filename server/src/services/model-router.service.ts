import { Capability, type LLMProvider, type ChatMessage, type ChatResponse } from '../providers/provider.interface.js';
  ...
  for (const slot of chain) {
    ...
    try {
      if (await provider.isAvailable()) {
        return await provider.chat(slot.model, messages);
      }
    } catch {
      // Try next in chain
    }
...
export async function* routeChatStream(
  ...
  for (const slot of chain) {
    ...
    try {
      if (await provider.isAvailable()) {
        return yield* provider.chatStream(slot.model, messages);
      }
    } catch {
      // Try next in chain
    }
